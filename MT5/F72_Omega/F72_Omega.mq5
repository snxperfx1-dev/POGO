...
//==================================================================
//= MODULE: F60Substrate  (HyperOmega — AUTHENTIC F60 spine)
//   The authoritative producers of physics / structure / phase /
//   space, per OMEGA-F72 spec STRATUM A. Replaces OMEGA's old proxy
//   substrate: Engine 1A now reads the real f_se 14-phase machine
//   (not the emergent node), and the Invisible Network is the real
//   FU-node registry (not a per-TF flip proxy). Everything above
//   (ERF/FRZ/RIE/MCE/NE/TQE/TE/IE2/DOE/Senseei/Risk/Execution) is
//   preserved and re-pointed to consume these authentic outputs.
//==================================================================
input group "═══ F60 Substrate (authentic spine) ═══"
input double InpF60WickFrac    = 0.30;   // FU spike: min wick / range
input int    InpF60Lookback    = 3;      // FU spike: structure lookback
input int    InpF60AuthMin     = 45;     // Min node authority
input int    InpF60NodeMax     = 250;    // Max remembered nodes
input int    InpF60PivotLen     = 5;     // f_se pivot length
input int    InpF60AtrLen       = 14;    // ATR length
input int    InpF60EffLen       = 10;    // efficiency lookback
input int    InpF60StructLen    = 10;    // structure pivot length
input double InpF60ImpulseAtr   = 1.5;   // impulse ATR multiple
input double InpF60EffThresh     = 0.65; // efficiency threshold
input double InpF60DispThresh    = 1.5;  // displacement ATR threshold
input double InpF60ConvMult      = 0.01; // convexity ATR multiplier
input double InpF60ChochBuf      = 0.75; // direction CHoCH buffer (ATR)
input int    InpF60InducLB       = 80;   // inducement lookback bars
input int    InpF60WarmupBars    = 1500; // history bars to warm each TF engine

// ── substrate parameter globals ──
...
int    f60_fractalStackDir=0;
double f60_fractalStackScore=0.0;
int    structBias=0;
string ie1a_currentPhase = "Point 4 Origin";
double ie1a_phaseConfidence = 20.0;
int    f60_waveDir=0, f60_stackDir=0;
double f60_stackPct=0.0;
double f60_waveOrigin=0.0, f60_waveObj=0.0;
...
int g_lookbackBarsFor(ENUM_TIMEFRAMES tf)
  {
   ...
   if(sec >= PeriodSeconds(PERIOD_D1)) return MathMin(InpF60WarmupBars, 400);
   if(sec >= PeriodSeconds(PERIOD_H4)) return MathMin(InpF60WarmupBars, 800);
   return InpF60WarmupBars;
  }
...
bool f_updateStructure()
  {
   ...
   f60_fractalStackDir = sb > sr ? 1 : sr > sb ? -1 : 0;
   f60_fractalStackScore = MathMax(sb, sr) / 6.0 * 100.0;
   ...
   ie1a_phaseConfidence = f_clamp(f60_fractalStackScore * 0.50 + g_se5.o.mf * 0.30 + g_se5.o.wp * 0.20, 20.0, 100.0);

   // legacy aliases
   f60_waveDir = l0_dir; f60_stackDir = f60_fractalStackDir; f60_stackPct = f60_fractalStackScore;
   f60_waveOrigin = g_se5.o.inv;
   return true;
  }
...
//--- F60 substrate parameter resolution + engine init -------------
void f60_substrate_init()
  {
   g_pivotLen=InpF60PivotLen; g_atrLen=InpF60AtrLen; g_effLen=InpF60EffLen; g_structLenL=InpF60StructLen;
   g_acceptBars=2; g_obMaxBars=50; g_inducLookback=InpF60InducLB; g_liqSweepLookback=10;
   g_resetBars=20; g_beliefSmooth=3; g_pivLen=8; g_lookback=InpF60Lookback; g_authMin=InpF60AuthMin;
   g_nodeMax=InpF60NodeMax; g_dormantBars=120; g_historyBars=600; g_minConf=55;
   g_impulseAtrMult=InpF60ImpulseAtr; g_effThresh=InpF60EffThresh; g_dispThresh=InpF60DispThresh;
   g_convMult=InpF60ConvMult; g_chochBufferATR=InpF60ChochBuf; g_inducZoneWidth=0.25;
   g_liqRadius=0.25; g_liqAgDecay=0.95; g_wickFrac=InpF60WickFrac;
   g_useStrictStruct=true; g_requireLiqSweep=true;
   f_buildLadder();
...
public:
   ...
   void Update(OmegaCurve &curve)
     {
      // HyperOmega §6 — re-point to the AUTHENTIC Invisible Network registry
      // (g_netBias/g_pdir/g_pressure/g_eligibleNodes), the real FU-node engine
      // built in the F60 substrate, instead of the per-TF flip proxy.
      netBias       = g_netBias;
      pdir          = g_pdir;
      pressure      = g_pressure;
      eligibleNodes = g_eligibleNodes;
     }
   void Update_LEGACY_PROXY(OmegaCurve &curve)
     {
...
public:
   ...
   void Update(OmegaCurve &curve)
     {
      // HyperOmega L1/§5.4 — phase authority is the AUTHENTIC f_se 14-phase
      // machine (g_se5 via f60 substrate global ie1a_currentPhase), NOT the
      // emergent curve-node state. The node phase remains in curve.tree as an
      // OBSERVATIONAL cross-reference only.
      currentPhase = (ie1a_currentPhase != "" ? ie1a_currentPhase : "Point 4 Origin");
...
//--- 9. Meta (Phase 6): SelfObservation + Probability cloud + Regime.
   ...
   //--- STRATUM A — authentic F60 substrate (the spine). Stand it up BEFORE
   //    the observers so Engine 1A reads the real f_se 14-phase machine and
   //    the Invisible Network is the real FU-node registry (OMEGA-F72 §8.1).
   f60_substrate_init();
   OmegaLogger::LogInfo("F60", StringFormat(
      "F60 substrate online · ladder %s/%s/%s/%s/%s/%s · f_se 14-phase = lifecycle authority",
      f_tfLabel(g_wtf1),f_tfLabel(g_wtf2),f_tfLabel(g_wtf3),
      f_tfLabel(g_wtf4),f_tfLabel(g_wtf5),f_tfLabel(g_wtf6)));
...
//    DeriveTrinity() folds them upward.
   if(g_curve.Update())
     {
      //--- STRATUM A — run the authentic F60 substrate FIRST each bar so all
      //    observers below consume the real f_se 14-phase phase + authentic
      //    Invisible Network this same cycle (OMEGA-F72 L2 upward-only flow).
      f60_substrate_update();
